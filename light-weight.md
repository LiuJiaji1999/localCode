...
工业质检“四座大山”:
# https://mp.weixin.qq.com/s/4fhAmwhMGjZa5fg3m0eLpg
1.缺陷检测的四大“拦路虎” :样本少，类别不平衡，缺陷成像不全（视角）、导入周期长
2.方案：有监督+无监督
## 引擎一：YOLO + 模拟光学成像
    有监督部分我们采用YOLO系列算法，但关键在于‘数据增强的创新’和‘模型结构的深度优化’。
比如：模拟光学成像数据增强——传统数据增强只是简单旋转、缩放，我们则引入了更贴近工业场景的增强策略：
...
同时配合'迁移学习'和'Focal Loss'，进一步缓解样本失衡问题，让模型更加关注难分类的缺陷样本。
...
[📑 Yao X, Li R, Zhang J, et al. Explicit boundary guided semi-push-pull contrastive learning for supervised anomaly detection[C]//Proceedings of the IEEE/CVF conference on computer vision and pattern recognition. 2023: 24490-24499.]

## 引擎二：SubspaceAD，无训练检出未知缺陷 
~SubspaceAD是埃因霍温理工大学团队在CVPR 2026提出的创新方法，其核心理念极其简洁：用冻结的DINOv2提取patch特征，再用PCA拟合正常样本的低维子空间，推理时计算重建残差即可定位异常。
[Lendering C, Akdag E, Bondarev E. SubspaceAD: Training-Free Few-Shot Anomaly Detection via Subspace Modeling[J]. arXiv preprint arXiv:2602.23013, CVPR 2026. ]
...
这套流程的核心优势在于：
    无监督模型让系统在“零缺陷样本”的情况下就能上线运行，边生产边学习，彻底打破了“先收集缺陷再上线”的传统路径依赖。
## 在具体运行时：
