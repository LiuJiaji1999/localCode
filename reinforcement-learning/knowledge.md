...
```bash
5️⃣ 20251205CCF会议： ADL166 世界模型基础与应用
1.世界模型的基础问题
2.世界模型驱动的具身智能系统
构建世界模型的几何表征方法
...
2D 激光雷达、3D 激光雷达 -- 激光 SLAM
相机+IMU -- 视觉惯导 SLAM(VLO)
激光雷达+IMU -- 激光惯导 SLAM(LIO)

视觉 SLAM 为例，算法框架如下图所示，主要包括：
    传感器信息读取、前端（里程计）、后端、回环检测和建图。
...
回环检测
    判断机器人是否到达过先前的位置，如果检测到回环，可以提供时隔更加久远的约束，可以消除累积误差，主要是通过图像之间的相似性来判断。
    最经典的方法是词袋模型、后续的基于CNN和RNN的方法可以有效提高回环检测的效率。
建图
...
·室内场景的三维重建（AR技术等）：通过图像和其他传感器信息计算相机的位置和三维环境结构，并结合3D渲染提供更自然的人机交互能力。
