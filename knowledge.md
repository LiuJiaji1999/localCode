...
![Alt text](./image/橡皮筋-AI的底层逻辑.png)
...
• 学习偏差：可以通过适当选择损失函数、约束和推理算法来引入，这些可以调节ML模型的训练阶段，明确支持收敛到符合底层物理的解决方案。通过使用和调整这些软惩罚约束（soft penalty constraints），底层物理定律只能近似满足；然而，这提供了一个非常灵活的平台，用于引入可以表达为积分、微分甚至分数方程的广泛物理偏差。
    从不同角度赋予NN先验知识，而不是设计专门的架构来隐含地强制执行这种知识，当前的努力旨在通过适当地惩罚传统NN近似的损失函数来以软方式施加这些约束。这种方法可以被视为多任务学习的一个特定用例，其中学习算法同时受到约束以拟合观测数据，并产生近似满足给定一组物理约束（例如，质量守恒、动量守恒、单调性等）的预测。代表性的方法有深度伽辽金法（deep Galerkin method）和PINNs 及其变体。
...
SFT 的核心理念在于，我们并非从零开始教模型新的知识，而是重塑现有知识的应用方式。
    预训练模型已经理解语言和语法，并吸收了大量的事实信息。
    SFT 将这种通用能力集中于特定的应用模式、响应风格和任务特定要求。
        这种方法之所以有效，是因为它利用了预训练期间学习到的丰富表征，同时所需的计算资源远少于从零开始训练。
        模型能够学习识别指令模式、保持对话上下文、遵循安全准则，并以期望的格式生成响应。

直接偏好优化 (DPO) 偏好对齐则更进一步训练模型生成符合人类偏好的响应。简而言之，它使语言模型更适用于现实世界的应用。偏好对齐解决了人工智能开发中的几个关键挑战。
