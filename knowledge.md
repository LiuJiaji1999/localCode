...
🔟 开放词汇 OVOD /开放世界 OWOD
开放词汇目标检测（Open-Vocabulary Object Detection, OVOD）关注：能不能检测“我用文本指定的新类别”。
开放世界目标检测（Open-World Object Detection, OWOD）关注：能不能发现“训练时没见过、我也没提前告诉你的未知物体”，并在之后逐步学习它。
    OVOD 描述为检测超出预定义类别的对象；OWOD 则强调在动态环境中识别已知类、发现未知类，并在获得新标签后增量学习；
1.数据：
OVOD 训练数据通常包括：
·有框标注数据，如 COCO、LVIS base classes	，用于学习定位和基础检测能力
·图文对数据，如 image-text pairs	用于学习视觉区域和语言词汇的对齐
·类别文本、prompt、CLIP 文本嵌入	用于让模型具备“看到词就能分类”的能力
OVOD 经常借助视觉-语言模型，比如 CLIP，把区域特征和文本类别嵌入对齐。DetCLIPv2、CORA 等工作都属于这类思路：通过区域-词语对齐、CLIP 适配或大规模图文预训练提升开放词汇检测能力。
---
OWOD 的训练数据通常按阶段组织：
阶段	                    已知类别	            未知类别
Task 1	            一部分已知类有标签	        其他类别可能出现在图像里，但没有标签
Task 2 / 3 / ...	新类别被加入已知类	         仍有新的未知类存在
OWOD 的关键不是“给模型一个类别词表让它找”，而是：图像中可能出现训练时没标注、没命名的物体，模型要把它们作为 unknown 检出来。经典 OWOD 定义还要求：当 oracle 后续给出这些 unknown 的标签后，模型能增量学习新类别。

2.训练
OVOD：
#输入：
图像、已知类别框标注或伪框标注、类别名称文本（例如 “dog”, “traffic light”）、有时还有图文对 caption
#输出/监督：
bounding-box、与类别文本对齐的分类分数、region-text matching score
#模型学的是：
“这个区域”和“这个文本类别”是否匹配。
所以分类器往往不是固定的 N 类 softmax，而是区域特征和文本特征的相似度。
---
OWOD
## 输入：
图像、当前阶段已知类的框标注、图像中可能含有未标注的新物体、后续阶段会新增类别标签
#输出/监督：
已知类检测框和类别、unknown-物体框、增量学习后的新类别预测
# 模型学的是：
“哪些是我认识的已知类，哪些看起来像物体但不属于已知类”。

3.推理
OVOD
input: image + ["cat", "bicycle", "fire extinguisher", "microscope"]
output: [box1, "fire extinguisher", 0.78] [box2, "microscope", 0.63]
---
OWOD
input: image
output: [box1, "car", 0.91] [box2, "person", 0.86] [box3, "unknown", 0.74]

| 维度         | OVOD                     | OWOD                       |
| ---------- | ------------------------ | -------------------------- |
| 目标         | 检测任意文本指定类别               | 检测已知类 + 发现未知类              |
| “未知类”是否有名字 | 有，推理时文本给出                | 没有，只能叫 unknown             |
| 依赖语言模型     | 很强，常用 CLIP/VLM           | 不一定，但近年来也会结合 VLM           |
| 是否增量学习     | 通常不是重点                   | 是核心要求之一                    |
| 主要难点       | 区域-文本对齐、未见类泛化、定位 novel 类 | unknown 召回、已知/未知区分、增量学习、遗忘 |
| 输出标签       | 给定词表中的类别                 | known 类别或 unknown          |
| 典型场景       | “帮我找图里的消防栓/显微镜/无人机”      | “自动驾驶中遇到没见过的障碍物，要先发现它”     |

🔟1️⃣ LLM 
encoder-only：
    在每次计算过程中，注意力层都能访问整个句子的所有单词，这些模型通常具有“双向”（向前/向后）注意力，被称为自编码模型。
    这些模型的预训练通常会使用某种方式破坏给定的句子（例如：通过随机遮盖其中的单词），并让模型寻找或重建给定的句子。
    “编码器”模型适用于需要理解完整句子的任务，例如： 句子分类、命名实体识别（以及更普遍的单词分类）和阅读理解后回答问题。
decoder-only：
    在每个阶段，对于给定的单词，注意力层只能获取到句子中位于将要预测单词前面的单词。这些模型通常被称为自回归模型。
    “解码器”模型的预训练通常围绕预测句子中的下一个单词进行。
    这些模型最适合处理 文本生成 的任务。
encoder-decoder：
    在每个阶段，编码器的注意力层可以访问输入句子中的所有单词，而解码器的注意力层只能访问位于输入中将要预测单词前面的单词。
    这些模型的预训练可以使用训练编码器或解码器模型的方式来完成，但通常会更加复杂。例如， T5 通过用单个掩码特殊词替换随机文本范围（可能包含多个词）进行预训练，然后目标是预测被遮盖单词原始的文本。
    序列到序列模型最适合于围绕根据给定输入生成新句子的任务，如 摘要、翻译或生成性问答。
tokenizer可以处理的原子操作：分词、转换为 ID 、 ID 转换回字符串、填充、截断以及注意力掩码

后训练方法：
特征        SFT             DPO
输入：标注图像-文本；   图像-文本 + 偏好排序输出
损失：标准监督损失；    基于偏好损失
目标：任务特定适应；    与人类一致的输出
应用场景：领域专业化；  创意性、主观性或多项选择任务

监督式微调 (SFT) 是指在特定任务的带标注示例数据集上持续训练预训练模型的过程。可以将其理解为专门教育：
    预训练教会模型通用的语言理解能力（例如学习阅读）。
    监督式微调教会模型特定的技能和行为（例如学习完成特定任务）。
SFT 的核心理念在于，我们并非从零开始教模型新的知识，而是重塑现有知识的应用方式。预训练模型已经理解语言和语法，并吸收了大量的事实信息。SFT 将这种通用能力集中于特定的应用模式、响应风格和任务特定要求。这种方法之所以有效，是因为它利用了预训练期间学习到的丰富表征，同时所需的计算资源远少于从零开始训练。模型能够学习识别指令模式、保持对话上下文、遵循安全准则，并以期望的格式生成响应。

直接偏好优化(DPO)偏好对齐则更进一步训练模型生成符合人类偏好的响应。它使人工智能系统更贴近人类的实际需求，而不仅仅是机械地执行指令。简而言之，它使语言模型更适用于现实世界的应用。偏好对齐解决了人工智能开发中的几个关键挑战。
    经过偏好对齐训练的模型在多个方面都表现出更佳的行为。它们产生的有害、带有偏见或不恰当的响应更少，输出也更实用，更贴近人类的实际需求。
